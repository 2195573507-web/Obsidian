...
Updated: 2026-07-07T15:02:08+00:00
...
- **[success/shared]** 2026-06-27 用户确认 GitHub push/PR/issue 和 MCP/Docker/DB 写入方向直接开做。Atlas Control 新增 guarded-execute：/api/write-capabilities、POST /api/github/write-action、POST /api/docker/action、POST /api/sqlite/write。GitHub 限 /root/.openclaw/workspace 与 /root/obsidian-vault，需显式 files、secret scan clean、confirm=true，risky files 阻断；Docker 限 sub2api/upstream-hub-standalone/sub2api-postgres/sub2api-redis，inspect/logs 只读，start/stop/restart 需 confirm；SQLite 限 collab/provider-watch/hermes monitor DB，写入需 confirm 且自动备份，DROP/TRUNCATE/ATTACH/DETACH/VACUUM/REINDEX/ALTER/writable_schema 阻断。QQ 新增 /写入能力 /docker操作预案 /数据库写入预案 /github写入预案。验证：py_compile 通过，service active，Docker inspect 成功 restart 无 confirm 阻止，非白名单容器 403，SQLite select 成功 update 无 confirm 阻止 drop 403，GitHub 无 files 阻止，nginx -t successful。
  - id: `ab1f74e0-4918-4615-bb6b-0282203c9c1a`; source: `openclaw`; importance: `88`; sensitivity: `shared`; tags: `atlas-control, guarded-execute, github, docker, sqlite, write-capabilities`
- **[success/shared]** 2026-06-27 用户追问 Hermes 方面控制。Atlas Control 新增 /api/hermes/status?lines=100，只读读取 hermes-gateway.service systemd 状态、MainPID、psutil 资源、journal warning/error 与 QQBot/WebSocket/slash command 摘要并脱敏。新增 /api/action-plan/restart-hermes，mode=report-only、execute=false、confirmation_required=true。QQ 命令新增 /hermes状态 与 /重启hermes；验证通过：hermes 状态 active/running，/重启hermes 返回操作卡不重启，py_compile 通过，atlas-control active，nginx -t successful。
  - id: `d174d0b3-45c3-4cb8-8a9e-08f980226f33`; source: `openclaw`; importance: `86`; sensitivity: `shared`; tags: `hermes, atlas-control, qq-command, report-only, gateway`
...
- **[success/shared]** 2026-06-27 继续 Hermes/OpenClaw 智能增强路线图：Atlas Control 新增 /重启openclaw、/压测中转、/同步github，但全部为 report-only 操作卡，execute=false、confirmation_required=true，不自动重启、不压测、不 push。新增 /api/action-plan/{restart-openclaw,provider-stress,github-sync}。新增只读 /api/error-summary?lines=80，从 Atlas 服务 journal 聚合 traceback/timeout/http_5xx/auth/connection/preflight 等错误类型并脱敏；/最近错误 已接入 error_summary。验证：py_compile 通过，atlas-control active，三个高风险 QQ 命令 ok=true 且不执行变更，/api/qq-commands 共 12 条，/api/error-summary ok=true，nginx -t successful。
  - id: `858c5224-cffc-425c-8db4-57c9a5770a9e`; source: `openclaw`; importance: `85`; sensitivity: `shared`; tags: `atlas, qq-command, report-only, error-summary, observability`
- **[success/shared]** 2026-06-27 继续 Hermes/OpenClaw 智能增强路线图：Atlas Collab 新增 web-research/qq-web-research job，DuckDuckGo HTML 只读搜索、公开网页 GET 抓取、生成本地 Obsidian 报告；不登录、不表单、不浏览器 profile、不外部写入。新增 SSRF 防护：仅 http/https、阻止 localhost/.local/metadata/private/loopback/link-local/reserved IP、非标准端口、redirect 后复查；create_job 增加 kind 白名单。Atlas Control QQ 新增 /研究 xxx，暴露 risk=external-read、external_network=true、methods_allowed=[GET] 等元数据，敏感 query 阻断。验证：py_compile 通过，服务 active，bad-kind 400，敏感查询 blocked，/研究 OpenClaw memory RAG job done risk=warning blocked=false safety_profile=web-research.v1 external_write=false report_exists=true，nginx -t successful。
  - id: `fe7fa12a-4f05-4eb5-b94e-e3b79a3af7ff`; source: `openclaw`; importance: `84`; sensitivity: `shared`; tags: `atlas, web-research, collab, qq-command, ssrf, report-only`
- **[success/shared]** 2026-06-27 继续路线图，Atlas Control QQ_COMMANDS 新增 /状态、/最近错误、/生成日报；/状态 聚合 Control overview + Collab status；/最近错误 聚合 self-check findings、provider alerts、failed queue；/生成日报 创建 Atlas Collab qq-daily job。Atlas Collab 新增 daily/qq-daily kind，读取 Atlas Daily /api/summaries，并走已有 preflight。验证通过：/状态 ok=true，/最近错误 ok=true，/生成日报 创建并完成 qq-daily job risk=warning blocked=false；回归 /排队记忆 成功；/api/qq-commands 列出 9 条命令；nginx -t successful。
  - id: `c1a9a139-004f-4cef-8e7d-e6c86e27e072`; source: `openclaw`; importance: `84`; sensitivity: `shared`; tags: `atlas, qq-command, collab, daily, recent-errors, status`
...
- **[success/shared]** 2026-06-27 继续路线图任务队列中心：Atlas Collab 新增 GET /api/queue/jobs/{id}/notify-preview 和 /recovery-suggestion。notify-preview mode=dry-run external_send=false，只生成 would-send 消息，不发 QQ/Telegram/Signal；recovery-suggestion mode=report-only execute=false，对 failed/blocked/timeout/unsupported kind/safety block/web-research 等场景给恢复建议。Atlas Control 增加同名代理端点。验证：py_compile 通过，服务 active，Collab 与 Control 代理均 ok，done job notify-preview would_send=true 但 external_send=false，recovery-suggestion 返回 no recovery needed，nginx -t successful。
  - id: `83b4523b-984c-49a6-9e98-d340d44e47a6`; source: `openclaw`; importance: `82`; sensitivity: `shared`; tags: `atlas-collab, queue, notify-preview, recovery-suggestion, report-only`
- **[success/shared]** 2026-06-27 继续路线图模型路由系统：Atlas Control 新增 /api/route-plan?task=... 和 QQ /模型建议 xxx。按任务描述分类 high_risk_ops/coding/research/memory/summarize/general，并给出 report-only 路由建议：高风险走强推理远程主模型+人工确认，研究走 web-research 队列，记忆先 Atlas Recall，摘要可辅助模型草稿主模型审定。输出 provider_context 和 cautions，不修改 provider 配置，不自动切换 Hermes/OpenClaw 主回复路由。验证：py_compile 通过，atlas-control active，/api/route-plan ok mode=report-only execute=false，QQ /模型建议 两类样例 ok，nginx -t successful。
  - id: `9cab3c93-bef6-47c3-b0de-4b88bbcfdb9c`; source: `openclaw`; importance: `82`; sensitivity: `shared`; tags: `atlas-control, model-routing, report-only, qq-command`
...
- **[success/shared]** 每日开源学习 2026-07-07：完成 12 个开源项目学习，覆盖 Beszel、Glance、Uptime Kuma、Langfuse、LiteLLM、Helicone、Open WebUI、Coolify、Portainer、Netdata、Homarr、one-api。高价值结论：小服务器监控优先轻量只读 dashboard；LLM 网关应分离 routing 与 observability；SSE streaming 需独立记录首包、断流、超时；高风险 Docker/systemd/GitHub/DB 动作保持 report-only/confirm/audit；Dashboard 首页适合 Glance/Homarr 风格扫读。报告路径：/root/daily-open-source-learning/2026-07-07/
  - id: `987b1ddc-39cb-481b-acb5-c9d901ebd910`; source: `hermes-cron`; importance: `7`; sensitivity: `shared`; tags: `learning, server, open-source`
- **[success/shared]** 每日开源学习 2026-07-05：研究 Beszel、Glance、Netdata、Uptime Kuma、Open WebUI、Langfuse、LiteLLM、Portkey Gateway、new-api、Coolify、shadcn/ui、TanStack Table。稳定结论：小服务器监控可优先轻量 agent+SQLite；LLM 网关需模型别名/渠道健康/fallback/预算/限流/审计；AI 调用观测需 trace/span/usage/cost/latency/error_type；Dashboard 可采用 Glance 卡片和 TanStack headless 表格；SSE/WebSocket 反代需专项检查但配置变更必须确认。报告路径：/root/daily-open-source-learning/2026-07-05/
  - id: `49f261a1-df69-43d3-9068-4ef50850aece`; source: `hermes-cron`; importance: `7`; sensitivity: `shared`; tags: `learning, server, open-source, observability`
