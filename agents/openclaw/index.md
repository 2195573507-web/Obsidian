---
type: index
category: agents
managed_by: obsidian-knowledge-web
updated_at: 2026-06-19T14:11:50
---
# agents/openclaw 索引

共 10 个文件。

- [[AGENTS]] ← `/root/.openclaw/workspace/AGENTS.md`
- [[IDENTITY]] ← `/root/.openclaw/workspace/IDENTITY.md`
- [[SOUL]] ← `/root/.openclaw/workspace/SOUL.md`
- [[TOOLS]] ← `/root/.openclaw/workspace/TOOLS.md`
- [[USER]] ← `/root/.openclaw/workspace/USER.md`
- [[workspace-AGENTS]] ← `/root/.openclaw/workspace/AGENTS.md`
- [[workspace-IDENTITY]] ← `/root/.openclaw/workspace/IDENTITY.md`
- [[workspace-SOUL]] ← `/root/.openclaw/workspace/SOUL.md`
- [[workspace-TOOLS]] ← `/root/.openclaw/workspace/TOOLS.md`
- [[workspace-USER]] ← `/root/.openclaw/workspace/USER.md`